...
import type { AssetCyberRiskDonutSegment } from "../utils/assetsByCyberRiskScoreDonutData.js";
...
export type CyberRisksByCyberRiskScoreDonutProps = {
  segments: readonly AssetCyberRiskDonutSegment[];
  sx?: object;
};

export default function CyberRisksByCyberRiskScoreDonut({
  segments,
  sx: sxProp,
}: CyberRisksByCyberRiskScoreDonutProps) {
  const { tokens } = useTheme();
  const active = useMemo(() => segmentsWithArcData(segments), [segments]);

  const totalCyberRisks = useMemo(
      ...
      <CardHeader
        title={
          <Typography variant="h4" component="h2" fontWeight={600}>
            Cyber risks by cyber risk score
          </Typography>
        }
        action={
          <Button
            variant="text"
            size="small"
            aria-label="More options for cyber risks by cyber risk score"
          >
